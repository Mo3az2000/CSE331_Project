...
// MAIN:
Node *PrintCompressedTree(string file_string, string pathName)
{
    ...
    fstream encodedFile;
    encodedFile.open(pathName +"_encodedFile.huff", ios::out);
...
string decode_file(struct Node *Tree, string ffs)
{
    ...
    string fs;
    for (auto element : fileByteArray)
        fs += one_byte_to_bitstring(element);
