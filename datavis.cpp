...
void visualize_graph(vector <user> users){
	fstream edgesFile;
	edgesFile.open("edges.txt",ios::out);
		...
		for(int i = 0; i < users.size(); i++)
		    {
		        for(int j = 0; j < users[i].followers.size(); j++)
		        {
		        	edgesFile << users[i].i.id << " ";
		        	edgesFile << users[i].followers[j].id << "\n";
		        }
		    }
	    }
}
...
int main()
{
    string file_name = "sample2.xml";
    print_graph(make_graph(make_tree(file_name)));
    visualize_graph(make_graph(make_tree(file_name)));
